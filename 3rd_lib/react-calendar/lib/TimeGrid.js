...
export default class TimeGrid extends Component {
  ...
  renderEvents(range, events, backgroundEvents, now) {
    let { components, accessors, localizer, dayLayoutAlgorithm } = this.props;

    const min = this.props.calendarStartTime;
    const max = this.props.calendarEndTime;
    const resources = this.memoizedResources(this.props.resources, accessors);
    const groupedEvents = resources.groupEvents(events);
    const groupedBackgroundEvents = resources.groupEvents(backgroundEvents);
    const viewCalendar = this.props.viewCalendar;
  
    return resources.map(([id, resource], i) => ( 
      
       range.map((date, jj) => {
      // console.log("llll",  new Date(moment(date).add(1, "days")));
        date  = viewCalendar === "week" ? new Date(moment(date).startOf("week").add(i, "days")) : date;
  ...
  render() {
    ...
    let start = viewCalendar === "week" ? new Date(moment(range[0]).startOf("week")) : range[0],
      end = viewCalendar === "week" ? new Date(moment(range[0]).endOf("week")) : range[range.length - 1];
    ...
    return (
      ...
      >
        <TimeGridHeader
          ...
          resizable={resizable}
          view={this.props.viewCalendar}
        />
        <div
          ref={this.contentRef}
          className="rbc-time-content"
          id="rbc-time-content-id"
        ...
        >
          <TimeGutter
            ...
            getters={getters}
            countEventsInHourSlot={countEventsInHourSlot}
          />
          {this.renderEvents(
            range,
            rangeEvents,
